...
document.getElementById('form_for_analyze').addEventListener('submit', function (event) {
  ...
  if (!Text) {
      ...
      //If the user does not have a value for the input when they submit, you should not continue processing and instead should inform them of the error on the page. 
      return;
  }

  const info = analyze_text(Text);
  return_results(info);
  document.getElementById('form_for_analyze').reset();
  //The form should reset itself every time after an input has been processed.
});

function analyze_text(Text) {
  const lowercased = Text.toLowerCase();
  //Lowercase the text in advance

  const letters = lowercased.match(/[a-z]/g) || [];
  //Total Letters: total number of letter characters in the text ,

  const nonLetters = lowercased.match(/[^a-z]/g) || [];
  //Total Non-Letters: total number of non-letters in the text (including spaces),

  const vowels = lowercased.match(/[aeiou]/g) || [];
  //Total Vowels: total number of vowels in the text (not counting y),

  const consonants = lowercased.match(/[bcdfghjklmnpqrstvwxyz]/g) || [];
  //Total Consonants: total number of consonants in the text (counting y),

  const words = lowercased.match(/[a-z]+/g) || [];
  //Total Words: total number of words in the text; a word is defined as any sequence of letters broken by any not-letter. For example, the phrase to-do is two words; a word does not start until a letter appears,
  ...
  return {
      ...
      totalNonLetters: nonLetters.length,
...
function return_results(info) {
  const dl = document.createElement('dl');

  for (const key in info) {
      const item = info[key];
      const helper = helper_key(key);

      const dt = document.createElement('dt');
      dt.textContent = helper + ':';
      dl.appendChild(dt);
